...
int main(){
    ...
    uniform_int_distribution<ll> dist_Z(0, 3);

    string ans = "";

    while((long double)(clock() - start) / CLOCKS_PER_SEC < 2.88) {
        string current = "";
        ...
        for(ll turn = 1; turn <= T; turn++) {
            ...
            compute_score(N, T, turn - 1, copiedTiles, bs);
            ...
            bs.assign(N, vector<bool>(N, false));
            ll score = compute_score(N, T, turn, copiedTiles, bs);
