    ...
    for(ll i = 1; i <= N; i++) {
        for(ll a = 0; a <= 300; a++) {
            for(ll b = 0; b <= 300; b++) {
                ...
                dp.at(i).at(a).at(b) = min(dp.at(i).at(a).at(b), dp.at(i - 1).at(max(a - A.at(i - 1), 0LL)).at(max(b - B.at(i - 1), 0LL)) + 1);
            }
        }
    }
    ll ans = INF;
...
ll solver2(ll N, ll X, ll Y, vector<ll> &A, vector<ll> &B) {
    vector<vector<vector<ll>>> dp(302, vector<vector<ll>>(301, vector<ll>(301, INF)));
    dp.at(0).at(0).at(0) = 0;
    for(ll i = 0; i <= N - 1; i++) {
        for(ll a = 0; a <= 300; a++) {
            for(ll b = 0; b <= 300; b++) {
...
int main(){
    ...
    while(true) {
       uniform_int_distribution<ll> dist_N(1, 300), dist_X(200, 300), dist_Y(200, 300), dist_A(200, 300), dist_B(200, 300);
       ll N = dist_N(rnd);
       ll X = dist_X(rnd);
       ll Y = dist_Y(rnd);
       vector<ll> A(N), B(N);
