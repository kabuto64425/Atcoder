...
void dfs(const Graph &G, ll v, vector<pair<ll, ll>> &timeStamps, ll &ptr, vector<bool> &seen, queue<ll> &ans) {
    //行きがけの処理を書く（タイムスタンプ・再帰終了条件など）
    timeStamps[v].first = ptr;
    ptr++;
    if(ans.empty() or ans.back() != v) {
        ans.push(v);
    }
    ...
    for (auto next_v : G[v]) { 
        ...
        dfs(G, next_v, timeStamps, ptr, seen, ans); // 再帰的に探索
    }
...
int main() {
    ...
    queue<ll> ans;
    dfs(G, 0, timeStamps, ptr, seen, ans);
